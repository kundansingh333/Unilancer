...
//       try {
//         const { otherUserId } = req.params;
//         const userId = req.userId; // ✅ now guaranteed by auth middleware
...
//         await Message.markAllAsRead(otherUserId, userId);
...
exports.getConversationWithUser = async (req, res) => {
  try {
    ...
    // ✅ REMOVED: The accidentally pasted 'markAllAsRead' function was here.
    // That code belongs in Section 5, not inside this function.
...
//       return res
//         .status(400)
//         .json({ success: false, error: "Cannot send message to yourself" });
//     }
...
//     // 🔥 FIX: Use standard Mongoose commands instead of custom methods
//     // Step A: Find existing conversation
//     let conversation = await Conversation.findOne({
//       participants: { $all: [req.userId, receiverId] },
//     });

//     // Step B: If no conversation, create one
//     if (!conversation) {
...
//       });
//     } else {
//       // Step C: If exists, update last message
...
//   } catch (err) {
//     console.error("Send message error:", err);
//     // This will now show the real error in your terminal if something else is wrong
//     res.status(500).json({ success: false, error: "Failed to send message" });
//   }
// };
...
exports.sendMessage = async (req, res) => {
  try {
    ...
    // 🔥 FIX: Use standard commands (findOne/create) instead of findOrCreate
    ...
    // 2. Socket Event
    if (global.io) {
      global.io.to(receiverId.toString()).emit("new-message", message);
    }
...
exports.markAllAsRead = async (req, res) => {
  try {
    ...
    if (!otherUserId) {
      return res
        .status(400)
        .json({ success: false, error: "Other user ID is required" });
    }

    // ✅ FIXED: Replaced custom static method with standard Mongoose updateMany
    // This directly updates the DB without relying on a model helper function
...
exports.editMessage = async (req, res) => {
  try {
    ...
    if (Date.now() - message.createdAt > fifteenMinutes) {
      return res
        .status(400)
        .json({ success: false, error: "Cannot edit after 15 minutes" });
    }
  ...
  try {
    ...
    if (deleteForEveryone) {
      ...
      if (Date.now() - message.createdAt.getTime() > oneHour) {
        return res
          .status(400)
          .json({ success: false, error: "Can only delete within 1 hour" });
      }
      await message.deleteForEveryone();
    } else {
      await message.deleteForUser(req.userId);
...
exports.addReaction = async (req, res) => {
  try {
    ...
    if (!emoji)
      return res
        .status(400)
        .json({ success: false, error: "Emoji is required" });
...
exports.removeReaction = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ success: false, error: "Failed to remove reaction" });
  }
};
...
exports.searchMessages = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ success: false, error: "Failed to search messages" });
  }
};
...
exports.muteConversation = async (req, res) => {
  try {
    const { otherUserId } = req.params;
    const { duration = 86400000 } = req.body;
    const conversation = await Conversation.findOrCreate(
      req.userId,
      otherUserId
    );
    ...
    res
      .status(500)
      .json({ success: false, error: "Failed to mute conversation" });
  }
};
...
exports.unmuteConversation = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ success: false, error: "Failed to unmute conversation" });
  }
};
...
exports.archiveConversation = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ success: false, error: "Failed to archive conversation" });
  }
};
...
exports.pinConversation = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ success: false, error: "Failed to pin conversation" });
  }
};
...
exports.setTyping = async (req, res) => {
  try {
    ...
    if (global.io) {
      global.io
        .to(otherUserId)
        .emit("typing", { userId: req.userId, isTyping });
    }
    ...
    res
      .status(500)
      .json({ success: false, error: "Failed to update typing status" });
  }
};
